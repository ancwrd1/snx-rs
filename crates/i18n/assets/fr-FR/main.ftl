...
button-close = Fermer
label-retrieve-info = Obtenir les infos du serveur
...
label-color-theme = Thème de couleur
...
label-no-cert-check = Désactiver toutes les vérifications de certificats TLS
...
label-client-cert = Certificat client ou chemin du pilote
...
label-mobile-access = Accès mobile
label-machine-cert-auth = Authentification par certificat machine
label-keychain-files = Fichiers du trousseau
label-all-files = Tous les fichiers
label-cancel = Annuler
label-open = Ouvrir
label-select-file = Sélectionner un fichier
label-ca-cert-files = Certificats X.509
label-allow-forwarding = Autoriser le transfert de paquets
...
error-cannot-acquire-access-cookie = Impossible d’obtenir le cookie d’accès !
...
error-invalid-cert-response = Réponse de certificat invalide
error-certificate-enrollment-failed = L'inscription du certificat a échoué, code d'erreur : {$code}
error-missing-cert-path = Chemin vers le fichier PKCS12 manquant !
error-missing-cert-password = Mot de passe PKCS12 manquant !
error-missing-reg-key = Clé d'inscription manquante !
...
error-device-not-found = Périphérique {$device} introuvable
...
cli-mobile-access-auth = Pour l'authentification de l'accès mobile, connectez-vous via l'URL suivante, puis trouvez un mot de passe utilisateur en format hexadécimal dans le code source HTML de la page et saisissez-le ici :
...
profiles-header = Profils de connexion
