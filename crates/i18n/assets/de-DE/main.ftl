...
label-no-cert-check = Alle TLS-Zertifikatsprüfungen deaktivieren
label-password-factor = Index des Passwortfaktors, 1..N
label-keychain = Benutzerpasswort im Schlüsselbund speichern
label-ike-lifetime = IPsec IKE SA-Lebensdauer, Sekunden
label-ike-persist = IPsec-IKE-Sitzung speichern
...
label-client-cert = Client-Zertifikatsdatei
label-driver-file = PKCS11-Treiberdatei
label-cert-password = PFX-Passwort
label-pkcs11-pin = PKCS11-PIN
label-cert-id = Hex-ID des PKCS11-Zertifikats
label-cert-name = Allgemeiner Name des Zertifikats
...
label-machine-cert-auth = Maschinenzertifikat-Authentifizierung
label-supported-files = Unterstützte Dateitypen
label-all-files = Alle Dateien
...
label-pin-required = PIN ist für die HSM-Geräteauthentifizierung erforderlich
label-pin = PIN
label-notification-level = Benachrichtigungsstufe
...
error-profile-not-found = Profil nicht gefunden: {$profile}
...
error-cannot-acquire-access-cookie = Zugriffscookie kann nicht abgerufen werden!
...
error-sysctl-not-converged = Sysctl ist nicht konvergiert: {$entry}
...
error-invalid-cert-response = Ungültige Zertifikatantwort
error-certificate-enrollment-failed = Zertifikatregistrierung fehlgeschlagen, Fehlercode: {$code}
error-missing-cert-path = Fehlender Pfad zum PKCS12-Zertifikat!
error-missing-cert-password = Fehlendes PKCS12-Passwort!
error-missing-reg-key = Fehlender Registrierungsschlüssel!
...
error-device-not-found = Gerät {$device} nicht gefunden
...
error-invalid-notification-level = Ungültige Benachrichtigungsstufe
error-unsupported-login-type = Nicht unterstützter Anmeldetyp
...
cert-type-pfx = PFX (PKCS12)-Schlüsselspeicher
cert-type-pem = PEM-Datei mit Zertifikat und privatem Schlüssel
cert-type-hw = HSM-Gerät
cert-type-system = Systemzertifikatsspeicher
...
# IKE versions
ike-version-autodetect = Automatisch erkennen
ike-version-1 = IKEv1
ike-version-2 = IKEv2

# Notification levels
notification-level-off = Aus
notification-level-minimal = Minimal
notification-level-standard = Standard
notification-level-verbose = Ausführlich

# Icon themes
theme-autodetect = Automatisch
theme-dark = Dunkel
theme-light = Hell
...
info-ike-lifetime = IKE-SA-Lebensdauer
info-ike-timestamp = IKE-SA-Zeitstempel
info-ike-expiration = Ablauf der IKE-SA
info-esp-spi-in = Eingehende ESP-SPI
info-esp-spi-out = Ausgehende ESP-SPI
info-esp-encryption-in = Eingehende ESP-Verschlüsselung
info-esp-authentication-in = Eingehende ESP-Authentifizierung
info-esp-encryption-out = Ausgehende ESP-Verschlüsselung
info-esp-authentication-out = Ausgehende ESP-Authentifizierung
...
cli-mobile-access-auth = Für die Authentifizierung des mobilen Zugriffs melden Sie sich bei { $url } an, suchen Sie anschließend ein Benutzerpasswort in hexadezimaler Form im HTML-Quelltext der Seite und geben Sie es hier ein.
cli-certificate-enrolled = Zertifikat wurde erfolgreich registriert.
cli-no-ike-state = Kein IKE-SA-Status!
cli-rekey-state-pending = Der IKE-SA-Status hat sich noch nicht geändert, es wird der vorherige Status angezeigt

# Connection Messages
connection-connected-to = Verbunden mit {$server}
connection-connecting-to = Verbindung zu {$server} wird hergestellt
connection-disconnected-from = Verbindung zu {$server} getrennt
connection-disconnecting-ike-expiry = Verbindung wird wegen Ablauf der IKE-SA bald getrennt
connection-rekeyed = Schlüssel des IPsec-Tunnels erfolgreich erneuert
...
# Connection profiles
profile-delete-prompt = Sind Sie sicher, dass Sie das ausgewählte Profil löschen möchten?
profile-default-name = Standard
profile-new-title = Neues Verbindungsprofil
profile-rename-title = Verbindungsprofil umbenennen
profiles-header = Verbindungsprofile
