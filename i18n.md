...
Every file also contains the entries which represent the supported locales, which start with `language-` prefix. For example: `language-en-US`, `language-de-DE`, etc. The base file for translation is `en-US/main.ftl` which contains English strings.

## Translation tasks

There are several defined translation tasks. Before proceeding, ask what task to perform, if not given explicitly. Do one task at a time.

## Task 1: adding a new locale support

The new locale will be further referred as `$LOCALE`. Ask for the exact locale name if it is not provided when given this task. Locale can be in the short form of xx-YY, or specified as a language. If it is unrecognized, do not proceed.

Add localization support for $LOCALE by doing the following:
...
* add a `language-$LOCALE` to all existing translation files and to the newly created file. Translate that entry in each of the files to the locale which this file represents.

## Task 2: synchronizing translations from the base English file with all other locales
...
* Remove entries from the locale files that are no longer present in the base file.
...
* Prefer efficient task execution via local Python or Bash scripts rather than consuming all translation files as tokens. 
* Do not modify the existing entries.
* Keep the original file structure intact, grouped by the key type.
* When adding new entries, add them to the corresponding group with the same prefix.
