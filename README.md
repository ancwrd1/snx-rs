...
| `failed to fill whole buffer`                                     | Some incoming IPSec UDP packets are blocked by the firewall because they are fragmented and bigger than MTU. Update your firewall settings to allow fragmented UDP packets through.      |
...
| `[0020] The user is not defined properly`                         | Application failed to negotiate IPSec IKE parameters. Please open a bug ticket with trace-level logs.                                                                                    |
